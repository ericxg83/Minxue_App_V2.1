...
const taskProcessing: Set<string> = new Set();
...
interface AnswerTask {
  ...
  questionId: string;
  status: 'pending' | 'processing' | 'completed' | 'failed';
  result?: { answer: string; explanation: string };
  error?: string;
  createdAt: Date;
  updatedAt: Date;
  questionText?: string; // 题目内容，用于生成答案和解析
}
...
const answerTaskQueue: AnswerTask[] = [];
const answerTaskProcessing: Set<string> = new Set();
...
仅返回 JSON 格式，确保 JSON 格式正确，不包含任何额外的文字：
{
  ...
  // API: Submit Batch Recognition Task
  app.post("/api/batch-recognize", async (req, res) => {
    try {
      ...
      if (images.length > 20) {
        return res.status(400).json({ error: "最多只能上传20张图片" });
      }
      ...
      res.json({ 
        success: true, 
        taskId, 
        message: `任务已创建，正在处理 ${images.length} 张图片` 
      });
    } catch (error: any) {
      console.error("Batch recognition error:", error);
      res.status(500).json({ error: error.message || "批量识别失败" });
    }
  });
      ...
      if (!supabase) {
        ...
        for (const q of questions) {
          ...
          if (exists) {
            ...
            // 创建答案生成任务
            const answerTaskId = `answer-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
            const answerTask: AnswerTask = {
              id: answerTaskId,
              questionId: id,
              status: 'pending',
              createdAt: new Date(),
            ...
            answerTaskQueue.push(answerTask);
          }
        }
  ...
  // API: Generate Answer for Question
  app.post("/api/generate-answer", async (req, res) => {
    try {
      const { questionId, questionText } = req.body;
      
      if (!questionId) {
        return res.status(400).json({ error: "题目 ID 不能为空" });
      }
      ...
      const answerTask: AnswerTask = {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        questionText: questionText // 存储题目内容，供任务处理时使用
      };
      ...
      res.json({ 
        success: true, 
        taskId: answerTaskId, 
        message: "答案生成任务已创建，正在处理" 
      });
    } catch (error: any) {
      console.error("Generate answer error:", error);
      res.status(500).json({ error: error.message || "生成答案失败" });
    }
  });

  // API: Get Answer Task Status
  app.get("/api/answer-task/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const task = answerTaskQueue.find(t => t.id === id);
      ...
      console.error("Get answer task status error:", error);
      res.status(500).json({ error: error.message || "获取任务状态失败" });
    }
  });
...
async function processTaskQueue() {
  while (true) {
    // 找到待处理的任务
    const pendingTask = taskQueue.find(task => task.status === 'pending' && !taskProcessing.has(task.id));
    
    if (pendingTask) {
      taskProcessing.add(pendingTask.id);
      ...
      try {
        ...
        pendingTask.error = error.message || '处理失败';
      } finally {
        taskProcessing.delete(pendingTask.id);
        pendingTask.updatedAt = new Date();
      }
    } else {
      // 没有待处理任务，等待一段时间后再次检查
...
// 处理答案生成任务队列
// 调用 AI 生成答案和解析
async function generateAnswerWithAI(questionText: string): Promise<{ answer: string; explanation: string }> {
  const qwenApiKey = process.env.QWEN_API_KEY || "sk-eac147ed202f4b93ba561c4a6e324874";
  const qwenModelId = process.env.QWEN_MODEL_ID || "qwen-vl-plus";
  
  const prompt = `你是一位资深的教育专家。请根据以下题目内容，生成标准答案和详细解析。
...
要求：
1. 答案要准确、简洁
2. 解析要详细，包括解题思路、关键步骤和知识点说明
3. 如果是选择题，要解释为什么正确选项是对的，以及其他选项为什么错
4. 如果是解答题，要给出完整的解题过程

请按以下 JSON 格式返回结果（不要包含任何其他文字）：
{
  "answer": "标准答案",
  "explanation": "详细解析内容"
}`;
  ...
  try {
    ...
    try {
      ...
      return {
        answer: result.answer || '生成失败',
        explanation: result.explanation || '生成失败'
      };
    } catch (e) {
      // 如果无法解析 JSON，返回原始内容
      return {
        answer: '生成失败',
        explanation: content || '生成失败'
      };
    }
  } catch (error: any) {
...
async function processAnswerTaskQueue() {
  while (true) {
    // 找到待处理的答案生成任务
    const pendingTask = answerTaskQueue.find(task => task.status === 'pending' && !answerTaskProcessing.has(task.id));
    
    if (pendingTask) {
      answerTaskProcessing.add(pendingTask.id);
      pendingTask.status = 'processing';
      pendingTask.updatedAt = new Date();
      
      try {
          // 获取题目内容
          const questionText = pendingTask.questionText || "这是题目内容";
          
          // 调用 AI 生成答案和解析
          const result = await generateAnswerWithAI(questionText);
          ...
          if (!supabase) {
            // Mock 模式：更新 mockDb
            const question = mockDb.find(q => q.id === pendingTask.questionId);
            if (question) {
              question.answer = result.answer;
              question.explanation = result.explanation;
              question.analysis = result.explanation; // 兼容前端使用的字段名
            }
          } else {
            // Supabase 模式：更新数据库
            const { error } = await supabase
              ...
              .update({
                ...
                analysis: result.explanation // 兼容前端使用的字段名
              })
              .eq('id', pendingTask.questionId);
            
            if (error) {
              console.error('更新答案到数据库失败:', error);
            }
          }
        } catch (error: any) {
          pendingTask.status = 'failed';
          ...
          answerTaskProcessing.delete(pendingTask.id);
          pendingTask.updatedAt = new Date();
        }
    } else {
      // 没有待处理任务，等待一段时间后再次检查
