...
// 注意：Vercel 环境变量有缓存问题，临时使用硬编码 API Key
...
// 可用模型：moonshotai/Kimi-K2.5 (视觉多模态，无严格速率限制)
...
正确格式示例：
{"questions":[{"number":"1","box":{"x":10,"y":20,"width":80,"height":15},"text":"1. 2+2=？A. 3 B. 4 C. 5 D. 6","stem":"2+2=？","options":["A. 3","B. 4","C. 5","D. 6"],"studentAnswer":"B","hasImage":false,"type":"choice"},{"number":"2","box":{"x":10,"y":40,"width":80,"height":15},"text":"2. 3+___=5","stem":"3+___=5","options":[],"studentAnswer":"2","hasImage":false,"type":"fill"}],"subject":"数学"}`;
...
仅返回 JSON 格式，确保 JSON 格式正确，不包含任何额外的文字：
{
  ...
  "options": ["A. 选项内容", "B. 选项内容", "C. 选项内容", "D. 选项内容"]
      ...
      const ocrBodyStr = JSON.stringify(ocrRequestBody);
      const ocrBodyBuffer = Buffer.from(ocrBodyStr, 'utf-8');
      ...
      response = await fetch(modelscopeEndpoint, {
        ...
        headers: {
          "Content-Type": "application/json; charset=utf-8",
          "Authorization": `Bearer ${modelscopeApiKey}`,
          "Content-Length": ocrBodyBuffer.length.toString()
        },
        body: ocrBodyBuffer,
        signal: controller.signal
      });
